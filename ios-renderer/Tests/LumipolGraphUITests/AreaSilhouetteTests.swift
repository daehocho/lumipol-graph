...
final class AreaSilhouetteTests: XCTestCase {
    ...
    // 0.30.0: 코어 출력 도메인 주입(구 AxisScale 대체)
    private let xDomain = AxisDomain(min: 0, max: 10)
    ...
    func testLayerSpansBottomFractionFromBaseline() {
        ...
        let layer = AreaSilhouette.layer(
            ...
            xDomain: xDomain, plotArea: plot, style: style
        )
    ...
    func testLayerClampsPointsOutsideXDomainToPlotRect() {
        ...
        let layer = AreaSilhouette.layer(
            ...
            xDomain: xDomain, plotArea: plot, style: .default
        )
    ...
    func testLayerAppliesStyleMinValueSpanToFlattenNoise() {
        ...
        let box = AreaSilhouette.layer(
            points: noise, xDomain: xDomain, plotArea: plot, style: .default
        )!.path!.boundingBox
        ...
        let noFloor = AreaSilhouette.layer(
            points: noise, xDomain: xDomain, plotArea: plot, style: flat
        )!.path!.boundingBox
    ...
    func testLayerNilForFewerThanTwoPoints() {
        XCTAssertNil(AreaSilhouette.layer(
            points: [AreaPoint(x: 0, y: 5)], xDomain: xDomain, plotArea: plot, style: .default
        ))
    }
}
