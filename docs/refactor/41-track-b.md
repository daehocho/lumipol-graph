...
- **SceneDigest(50 문서 §2) — 보류**: 렌더러 테스트 전용 덤프 인프라(양 플랫폼 직렬화 + diff 배선)는
  별도 세션 분량. 이번 트랙 B 전환분은 렌더러가 코어 출력을 직소비하도록 재배선돼(역산·재계산
  소거) 코어 골든 + 양 플랫폼 단위/스냅샷 테스트가 회귀를 커버한다. 색 해석값 대조(iOS 동적
  UIColor ↔ ChartDefaults 팔레트)가 SceneDigest의 남은 고유 가치 — 차기 증분 항목.
